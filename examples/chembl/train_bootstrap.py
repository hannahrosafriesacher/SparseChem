...
parser.add_argument("--bootstrap_indeces")
parser.add_argument("--currentIteration", help="for selecting the bootstrap_indeces", type=int)
...
print('i', args.currentIteration)
bootstrap_indeces=np.load(args.bootstrap_indeces)
bootstrap_indeces_now=bootstrap_indeces[args.currentIteration, ].tolist()
print(len(bootstrap_indeces_now))
ecfp     = sc.load_sparse(args.x)
y_class  = sc.load_sparse(args.y_class)
y_regr   = sc.load_sparse(args.y_regr)
y_censor = sc.load_sparse(args.y_censor)
print('---------------------------------------------------------------', 'ECFP', ecfp.shape, 'Y_CLASS', y_class.shape)
...
dataset_tr = sc.ClassRegrSparseDataset(x=ecfp[idx_tr][bootstrap_indeces_now,], y_class=y_class_tr[bootstrap_indeces_now,], y_regr=y_regr_tr, y_censor=y_censor_tr)
